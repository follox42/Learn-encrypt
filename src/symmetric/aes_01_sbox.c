/*
 * Challenge 1: AES S-Box (SubBytes)
 *
 * Goal: Implement the AES SubBytes transformation using the S-box
 *       lookup table. Each byte in the 4x4 state matrix is replaced
 *       by its corresponding S-box value.
 *
 * How it works:
 * - For byte value 0xAB: row = 0xA, column = 0xB
 * - state[row][col] = SBOX[ state[row][col] ]
 * - Can use a flat 256-byte lookup table: SBOX[byte_value]
 *
 * Tests:
 * sbox[0x00] = 0x63
 * sbox[0x53] = 0xED
 * sbox[0xFF] = 0x16
 *
 * Full state test:
 * Input:          After SubBytes:
 * 19 a0 9a e9    d4 e0 b8 1e
 * 3d f4 c6 f8    27 bf b4 41
 * e3 e2 8d 48    11 98 5d 52
 * be 2b 2a 08    ae f1 e5 30
 */

#include <stdio.h>

/* AES S-box (256 bytes) */
static const unsigned char SBOX[256] = {
    
};

/* Apply SubBytes to the entire 4x4 state */
void sub_bytes(unsigned char state[4][4])
{
    /* Your code here */
}

/* Print state matrix */
void print_state(const unsigned char state[4][4])
{
    /* Your code here */
}

int main(void)
{
    /* Individual byte tests */
    printf("=== Individual S-box lookups ===\n");
    printf("sbox[0x00] = 0x%02X (expected: 0x63)\n", SBOX[0x00]);
    printf("sbox[0x53] = 0x%02X (expected: 0xED)\n", SBOX[0x53]);
    printf("sbox[0xFF] = 0x%02X (expected: 0x16)\n\n", SBOX[0xFF]);

    /* Full state test */
    unsigned char state[4][4] = {
        {0x19, 0xA0, 0x9A, 0xE9},
        {0x3D, 0xF4, 0xC6, 0xF8},
        {0xE3, 0xE2, 0x8D, 0x48},
        {0xBE, 0x2B, 0x2A, 0x08}
    };

    printf("=== Full state SubBytes ===\n");
    printf("Before:\n");
    print_state(state);

    sub_bytes(state);

    printf("After SubBytes:\n");
    print_state(state);

    printf("Expected:\n");
    printf("  d4 e0 b8 1e\n");
    printf("  27 bf b4 41\n");
    printf("  11 98 5d 52\n");
    printf("  ae f1 e5 30\n");

    return 0;
}
